...
class GradioBatchTTS:
    ...
    def gradio_whisper(
        ...
        try:
            ...
            for file in files:
                ...
                denoise_inst_path, denoise_vocal_path = self._denoise_batch(
                    fm, denoise_level
                )
                input_path = (
                    denoise_vocal_path
            ...
            logger.error(
                f"[gradio_batch_tts.py] gradio_upload_source - Error transcribing file: {e}"
            )
            gr.Warning(f"{e}")
            return self.batch_manager.get_all_files()

    def _upload_file_batch(self, source_file, audio_format: str):
        ...
        logger.debug(
            f"_upload_file: source_file={source_file}, has_audio={has_audio}, has_video={has_video}"
        )
    ...
    def gradio_translate_batch(self, source_lang, target_lang):
        ...
        try:
            ...
            logger.error(
                f"[gradio_batch_tts.py] gradio_translate - Error transcribing file: {e}"
            )
            gr.Warning(f"{e}")
            return None
    ...
    def _translate_srt_batch(self, fm, source_lang, target_lang):
        ...
        self.translator.translate_file(
            source_lang, target_lang, srt_file, srt_translated_file
        )
    ...
    def gradio_dubbing_batch(
        self, voice_name: str, semitones, speed_factor, volume_factor, audio_format: str
    ):
        return None
