...
class GradioKara:
    ...
    def upload_source(
        ...
        try:
            ...
            return (
                self.fm.get_split("Source.video"),
                self.fm.get_split("Source.audio"),
                self.fm.get_all_files(),
            )
        except Exception as e:
            logger.error(
                f"[gradio_kara.py] upload_source - Error transcribing file: {e}"
            )
            gr.Warning(f"{e}")
            return None, None, None
    ...
    def update_demixing_models(self):
        ...
        local_dropdown = gr.Dropdown(
            label=i18n("MDX Models"),
            choices=local_models,
            value=self.user_config.get("demixing_model", "htdemucs"),
        )
        return local_dropdown

    # return Video, Audio, Video, Audio, File
    def demixing(self, demixing_model, audio_format):
        ...
        try:
            ...
            return (
                self.fm.get_split("Instrumental.video"),
                self.fm.get_split("Instrumental.audio"),
                self.fm.get_split("Vocals.video"),
                self.fm.get_split("Vocals.audio"),
                self.fm.get_all_files(),
            )
        except Exception as e:
    ...
    def _demixing(self, demixing_model, audio_format):
        ...
        if demixing_model in ["htdemucs", "htdemucs_6s", "htdemucs_ft", "mdx_extra"]:
            inst_audio_file, vocal_audio_file = demucs_split_file(
                input_audio_file, output_dir, demixing_model, audio_format
            )
        else:
            ...
            main_filepath, invert_filepath = run_mdx(
            ...
            inst_audio_file = os.path.join(
                path_workspace_folder(),
                file_name + f"_{demixing_model}_main." + audio_format,
            )
            vocal_audio_file = os.path.join(
                path_workspace_folder(),
                file_name + f"_{demixing_model}_invert." + audio_format,
            )
        ...
        if self.has_video:
            ...
            inst_video_file = path_add_postfix(
                input_video_file, f"_{demixing_model}_inst"
            )
            ...
            vocal_video_file = path_add_postfix(
                input_video_file, f"_{demixing_model}_vocal"
            )
    ...
    def transcribe(
        ...
        try:
            ...
            subtitles = self.whisper_inf.transcribe_file(
                input_path, params, highlight_words, gr.Progress()
            )
