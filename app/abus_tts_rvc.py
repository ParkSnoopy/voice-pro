...
class TTSRVC:
    ...
    def srt_to_voice(
        ...
        tts_audio_file = path_add_postfix(
            subtitle_file, f"-{tts_voice}", f".{audio_format}"
        )
        rvc_audio_file = path_add_postfix(
            subtitle_file, f"-{tts_voice}-{rvc_voice}", f".{audio_format}"
        )
        rvc_subtitle_file = path_add_postfix(
            subtitle_file, f"-{tts_voice}-{rvc_voice}", ".srt"
        )
        ...
        for i in progress.tqdm(range(len(subs)), desc="Generating..."):
            ...
            tts_segment_file = os.path.join(
                tts_segments_folder, f"tts_{i + 1:06}.{audio_format}"
            )
            if False == self.tts.line_to_voice(
                line.text,
                tts_segment_file,
                tts_voice,
                semitones,
            ...
            rvc_segment_file = os.path.join(
                rvc_segments_folder, f"rvc_{i + 1:06}.{audio_format}"
            )
            self.rvc.simple_inference(
                tts_segment_file, rvc_segment_file, rvc_voice, audio_format
            )
    ...
    def text_to_voice(
        ...
        tts_audio_file = os.path.join(
            path_dubbing_folder(), path_new_filename(ext=f".{audio_format}")
        )
        rvc_audio_file = path_add_postfix(
            tts_audio_file, f"-{rvc_voice}", f".{audio_format}"
        )

        self.tts.text_to_voice(
            text.strip(),
            tts_audio_file,
            tts_voice,
            semitones,
        ...
        self.rvc.simple_inference(
            tts_audio_file, rvc_audio_file, rvc_voice, audio_format
        )
    ...
    def infer(
        ...
        if subtitle_file:
            tts_audio_file, rvc_audio_file = self.srt_to_voice(
                subtitle_file,
                tts_voice,
                semitones,
                speed_factor,
            ...
            tts_audio_file, rvc_audio_file = self.text_to_voice(
                text,
                tts_voice,
                semitones,
                speed_factor,
