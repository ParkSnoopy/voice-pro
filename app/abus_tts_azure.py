...
class AzureTTS:
    ...
    def speech_synthesis_get_available_voices(self):
        ...
        logger.debug(
            "Enter a locale in BCP-47 format (e.g. en-US) that you want to get the voices of, or enter empty to get voices in all locales."
        )
        try:
            text = input()
        except EOFError:
        ...
        if result.reason == speechsdk.ResultReason.VoicesListRetrieved:
            ...
            print(
                "Speech synthesis canceled; error details: {}".format(
    ...
    def request_tts(
        ...
        output_voice_file = os.path.join(
            path_dubbing_folder(), path_new_filename(ext=".mp3")
        )
        ...
        if False == self.generate_audio(
            ...
            logger.warning(
                "[abus_tts_azure.py] request_tts - error: API returns False"
            )
            return False
