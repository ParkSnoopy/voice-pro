...
def cmd_copy_files(from_files: list, dest_directory: str) -> list:
    ...
    for from_file in from_files:
        ...
        try:
            ...
            logger.error(
                f"[abus_path.py] cmd_copy_files - Error copying {from_file}: {e}"
            )
            # 오류가 발생해도 계속 진행
            to_file = os.path.join(dest_directory, from_file_name)
            to_files.append(to_file)
...
def cmd_delete_file(target_file):
    ...
    try:
        ...
        logger.error(
            f"[abus_path.py] cmd_delete_file - Permission denied: {target_file}"
        )
...
def cmd_open_explorer(path="."):
    ...
    try:
        ...
        print("File explorer command not found.")
...
def cmd_select_folder(default_path="."):
    try:
        ...
        try:
            ...
            try:
                folder_path = subprocess.check_output(
                    ["kdialog", "--getexistingdirectory", default_path], text=True
                ).strip()
            except FileNotFoundError:
                folder_path = (
                    input(f"Enter folder path (default: {default_path}): ").strip()
                    or default_path
                )
