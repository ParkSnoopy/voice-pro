...
class WhisperXInference:
    ...
    def release_memory():
        gc.collect()
        logger.debug("[abus_asr_whisperx.py] release_memory - OK!! ")
    ...
    def available_compute_types():
        ...
        logger.debug(
            f"[abus_asr_whisperx.py] available_compute_types - types = {types} "
        )
    ...
    def transcribe_live(
        ...
        try:
            ...
            result, time_for_task = self.transcribe(
            ...
            logger.error(
                f"[abus_asr_whisperx.py] transcribe_live - An error occurred: {e}"
            )
            return None, None
        finally:
            self.release_memory()
    ...
    def transcribe_file(
        ...
        try:
            ...
            subtitles = self.generate_and_write_file(
                input_path, result, highlight_words
            )
            return subtitles
        except Exception as e:
            logger.error(
                f"[abus_asr_whisperx.py] transcribe_file - An error occurred: {e}"
            )
        finally:
            self.model = None
            self.release_memory()
    ...
    def update_model(self, params: WhisperParameters, progress: gr.Progress):
    ...
    def _denoise_live(self, input: np.ndarray, denoise_level: int = 0) -> np.ndarray:
        logger.debug(
            f"[abus_asr_whisperx.py] _denoise_live:input.shape = {input.shape}"
        )

        return input

    @staticmethod
    def generate_and_write_file(input_path: str, result, highlight_words: bool) -> list:
        subtitles = []
        ...
        try:
            ...
            logger.error(
                f"[abus_asr_whisperx.py] generate_and_write_file - An error occurred: {e}"
            )
