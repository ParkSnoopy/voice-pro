...
class YoutubeDownloader:
    ...
    def dl_progress_hook(self, d):
        ...
        try:
            total_bytes = (
                d["total_bytes"]
                if "total_bytes" in d
                else (d["total_bytes_estimate"] if "total_bytes_estimate" in d else 0)
            )
            downloaded_bytes = d["downloaded_bytes"]

            if d["status"] == "downloading" and total_bytes > 0:
                self.progress(
                    int(downloaded_bytes / total_bytes * 100) / 100.0,
                    desc="YouTube Downloader",
                )
        except Exception as e:
            logger.error(
                f"[abus_downloader.py] dl_progress_hook - An error occurred: {e}"
            )
    ...
    def yt_download(
        ...
        if quality == "best":
            ydl_opts["format"] = (
                "bestvideo[ext=mp4][vcodec^=avc1]+bestaudio[ext=m4a]/bestvideo[ext=webm][vcodec^=vp9]+bestaudio[ext=webm]/best"
            )
        elif quality == "good":
            ydl_opts["format"] = (
                "bestvideo[ext=mp4][vcodec^=avc1][height<=1080]+bestaudio[ext=m4a]/best[ext=mp4][vcodec^=avc1][height<=1080]/best"
            )
        elif quality == "low":
            ydl_opts["format"] = (
                "bestvideo[ext=mp4][vcodec^=avc1][height<=720]+bestaudio[ext=m4a]/best[ext=mp4][vcodec^=avc1][height<=720]/best"
            )
