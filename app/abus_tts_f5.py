...
class F5TTS:
    def __init__(self):
        config_path = os.path.join(
            Path(__file__).resolve().parent, "abus_tts_f5_models.json"
        )
    ...
    def load_f5tts(self, model_name: str = "SWivid/F5-TTS_v1"):
        ...
        vocab_path = (
            str(cached_path(model_paths["vocab_path"]))
            if "vocab_path" in model_paths
            else ""
        )
        ckpt_path = (
            str(cached_path(model_paths["model_path"]))
    ...
    def infer_multi(
        ...
        self.select_model(model_choice)
        ref_audio1, ref_text1 = preprocess_ref_audio_text(
            celeb_audio1, celeb_transcript1
        )
        ref_audio2, ref_text2 = preprocess_ref_audio_text(
            celeb_audio2, celeb_transcript2
        )
