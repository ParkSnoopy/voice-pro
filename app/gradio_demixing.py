...
class GradioDemixing:
    ...
    def upload_source(
        ...
        try:
            ...
            logger.error(
                f"[gradio_demixing.py] upload_source - Error transcribing file: {e}"
            )
            gr.Warning(f"{e}")
            return None, None, None
    ...
    def update_demixing_models(self):
        ...
        remote_dropdown = gr.Dropdown(
            label=i18n("MDX Models"), choices=remote_models, value=None
        )
    ...
    def demixing(self, demixing_model, audio_format):
        ...
        try:
            ...
            logger.error(
                f"[gradio_demixing.py] demixing - Error transcribing file: {e}"
            )
            gr.Warning(f"{e}")
    ...
    def _demixing(self, demixing_model, audio_format):
        ...
        if demixing_model in ["htdemucs", "htdemucs_6s", "htdemucs_ft", "mdx_extra"]:
            ...
            inst_audio_file = os.path.join(
                path_workspace_subfolder(input_audio_file),
    ...
    def show_model_info(self, demixing_model):
        ...
        if model:
            return gr.Textbox(
                ...
                value=model.download_info(),
                interactive=False,
            )
        else:
            return gr.Textbox(
                label=i18n("Model information"),
                value=i18n("Select a model to download"),
                interactive=False,
            )

    # return Dropdown, Dropdown, Textbox
    def download_model(self, demixing_model):
        # local_dropdown, remote_dropdown = self.update_demixing_models()
        model = AbusHuggingFace.hf_get_from_name(demixing_model)
        if model == None:
            return gr.Textbox(
                label=i18n("Download Status"),
                value=i18n("Download Error"),
                interactive=False,
            )
        ...
        if model.download():
            return gr.Textbox(
                label=i18n("Download Status"),
                value=i18n("OK - Please refresh models"),
                interactive=False,
            )
        else:
            return gr.Textbox(
