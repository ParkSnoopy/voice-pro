...
class GradioTTSRVC:
    ...
    def gradio_deep_voice(
        ...
        input_text = text
        logger.debug(
            f"[gradio_tts_rvc.py] gradio_deep_voice - input_text: {input_text}"
        )

        try:
            ms_voice = self.voice_manager.get_voice(tts_voice)

            tts_audio_file, rvc_audio_file = self.ttsrvc.infer(
                input_text.strip(),
                ms_voice.name,
                semitones,
                speed_factor,
    ...
    def gradio_default(self):
        rvc_voice = gr.Dropdown(
            ...
            choices=self.ttsrvc.get_voices(),
            value=self.user_config.get("rvc_voice"),
        )
