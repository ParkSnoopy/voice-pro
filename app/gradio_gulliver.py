...
class GradioGulliver:
    def __init__(self, user_config: UserConfig):
        ...
        self.ms_voice_manager = MSVoiceManager(
            self.user_config.get("ms_language", "English")
        )
        ...
        self.translator = (
            AzureTranslator() if azure_text_api_working() == True else DeepTranslator()
        )
    ...
    def update_whisper_models(self, asr_engine):
        ...
        if len(model_list) > 0:
            model_name = self.user_config.get(
    ...
    def gradio_upload_source(
        ...
        try:
            logger.debug(
                f"upload_source: file_obj={file_obj}, mic_file={mic_file}, youtube_url={youtube_url}"
            )
            self.fm = FileManager()
            if (
                self._upload(
                    file_obj, mic_file, youtube_url, video_quality, audio_format
                )
                == False
            ):
            ...
            logger.error(
                f"[gradio_gulliver.py] gradio_upload_source - Error transcribing file: {e}"
            )
            gr.Warning(f"{e}")
            return None, None, None
    ...
    def _upload(
        ...
        if file_obj is not None:
            uploaded_file = cmd_copy_file_to(
                file_obj.name, path_workspace_subfolder(file_obj.name)
            )
        elif mic_file and mic_file.strip():
            uploaded_file = cmd_copy_file_to(
                mic_file, path_workspace_subfolder(mic_file)
            )
        elif youtube_url and youtube_url.strip():
            youtube_file = self.downloader.yt_download(
                youtube_url, path_youtube_folder(), video_quality
            )
            uploaded_file = cmd_copy_file_to(
                youtube_file, path_workspace_subfolder(youtube_file)
            )
        else:
            return False
        ...
        logger.debug(
            f"upload_source: source_file={self.source_file}, has_audio={self.has_audio}, has_video={self.has_video}"
        )
    ...
    def gradio_whisper_default(self):
        return ["large", "english", self.get_whisper_default_compute_type(), 0]

    # return Video, Audio, File
    def gradio_whisper(
        self, asr_engine, modelName, whisper_language, compute_type, denoise_level
    ):
        ...
        try:
            source_audio = self.fm.get_split("Source.audio")
            denoise_inst_path, denoise_vocal_path = self._denoise(
                source_audio, denoise_level
            )
            input_path = (
                denoise_vocal_path
                if os.path.exists(denoise_vocal_path)
                else source_audio
            )
            ...
            self.whisper_inf = self.switch_case(asr_engine)
            subtitles = self.whisper_inf.transcribe_file(
                input_path, params, False, gr.Progress()
            ...
            logger.debug(
                f"srt_file = {srt_file}, self.source_file = {self.source_file}"
            )

            if self.has_video and ffmpeg_browser_compatible(self.source_file):
                if srt_file:
                    return (
                        (self.source_file, srt_file),
                        source_audio,
                        srt_string,
                        self.fm.get_all_files(),
                    )
                else:
                    return (
                        self.source_file,
    ...
    def _demucs_htdemucs(self, source_audio):
        ...
        inst_audio_file, vocal_audio_file = demucs_split_file(
            source_audio, output_dir, "htdemucs", extension[1:]
        )
    ...
    def _demucs_htdemucs_ft(self, source_audio):
        ...
        inst_audio_file, vocal_audio_file = demucs_split_file(
            source_audio, output_dir, "htdemucs_ft", extension[1:]
        )
        self.fm.set_split("Instrumental.audio", inst_audio_file)
        self.fm.set_split("Vocals.audio", vocal_audio_file)
    ...
    def gradio_translate(self, source_lang, transcription_text, target_lang):
        ...
        if self._is_subtitle_format(transcription_text):
            ...
            transcription_file = os.path.join(
                path_translate_folder(), path_new_filename(f".{subs.format}")
            )
        ...
        if transcription_file:
            translation_file = self._translate_subtitle(
                transcription_file, source_lang, target_lang
            )
        else:
        ...
        output_video_path = (
            (source_video_file, translation_file)
            if source_video_file and translation_file
            else source_video_file
        )
        output_audio_path = source_audio_file
        output_translation_text = (
            self._read_file(translation_file) if translation_file else translation_text
        )
        return (
            output_video_path,
            output_audio_path,
    ...
    def _translate_subtitle(self, subtitle_file, source_lang, target_lang):
        try:
            ...
            self.translator.translate_file(
                source_lang, target_lang, subtitle_file, translation_file
            )
    ...
    def gradio_edge_dubbing(
        ...
        logger.debug(
            f"[gradio_gulliver.py] gradio_edge_dubbing - \
        ...
        output_video_path = (
            (aidub_video_file, translation_file)
            if aidub_video_file and translation_file
            else aidub_video_file
        )
    ...
    def _edge_tts_text(
        ...
        try:
            ...
            # TTS
            source_audio_file = self.fm.get_split("Source.audio")
            aidub_audio_file = path_add_postfix(
                source_audio_file, f"_{target_language}"
            )

            self.edge_tts.text_to_voice(
                text,
                aidub_audio_file,
                ms_voice.name,
                semitones,
            ...
            # Mix
            mixed_audio_file = path_add_postfix(
                source_audio_file, f"_mixed_{target_language}"
            )
            ...
            if self.has_video:
                source_video_file = self.fm.get_split("Source.video")
                aidub_video_file = path_add_postfix(
                    source_video_file, f"_{target_language}"
                )

                ffmpeg_replace_audio(
                    source_video_file, mixed_audio_file, aidub_video_file
                )
    ...
    def _edge_tts_subtitle(
        ...
        logger.debug(
            f"[gradio_gulliver.py] _edge_tts_subtitle - subtitle_file = {subtitle_file}, voice_name = {voice_name}, \
                     semitones = {semitones}, speed_factor = {speed_factor}, volume_factor = {volume_factor}, audio_format = {audio_format}"
        )
    ...
    def gradio_f5_dubbing_single(
        ...
        logger.debug(
            f"[gradio_gulliver.py] gradio_f5_dubbing_single - \
        ...
        if len(translation_text) < 1:
            logger.warning(
                "[gradio_gulliver.py] gradio_f5_dubbing_single - no actions"
            )
            return None, None, self.fm.get_all_files()
        ...
        if translation_file:
            aidub_video_file, mixed_audio_file = self._f5_tts_single(
                self._read_file(translation_file),
                celeb_name,
                celeb_audio,
                celeb_transcript,
                model_choice,
    ...
    def _f5_tts_single(
        ...
        logger.debug(
            f"[gradio_gulliver.py] _f5_tts_single - text = {text}, \
                    celeb_name = {celeb_name}, celeb_audio = {celeb_audio}, \
                    model_choice = {model_choice}, speed_factor = {speed_factor}, audio_format = {audio_format}"
        )
        ...
        try:
            ...
            # Mix
            mixed_audio_file = path_add_postfix(
                source_audio_file, f"_mixed_{celeb_name}"
            )
            denoise_inst_path, _ = self._denoise(source_audio_file, 1)
            ffmpeg_mix_audio(
                aidub_audio_file,
    ...
    def _cosy_tts_single(
        ...
        logger.debug(
            f"[gradio_gulliver.py] _cosy_tts_single - text = {text}, \
                    celeb_name = {celeb_name}, celeb_audio = {celeb_audio}, \
    ...
    def gradio_kokoro_dubbing(
        ...
        logger.debug(
            f"[gradio_gulliver.py] gradio_kokoro_dubbing - \
        ...
        if translation_file:
            aidub_video_file, mixed_audio_file = self._kokoro_tts_subtitle(
                translation_file, language_name, voice_name, speed_factor, audio_format
            )
        elif translation_text:
            aidub_video_file, mixed_audio_file = self._kokoro_tts_text(
                translation_text, language_name, voice_name, speed_factor, audio_format
            )
    ...
    def _kokoro_tts_text(
        self, text: str, language_name, voice_name: str, speed_factor, audio_format: str
    ):
        try:
            # TTS voice
            ...
            self.kokoro_tts.text_to_voice(
                text, aidub_audio_file, kokoro_voice, speed_factor, audio_format
            )
            ...
            # Mix
            mixed_audio_file = path_add_postfix(
                source_audio_file, f"_mixed_{voice_name}"
            )
            denoise_inst_path, _ = self._denoise(source_audio_file, 1)
            ffmpeg_mix_audio(
                aidub_audio_file,
    ...
    def _kokoro_tts_subtitle(
        ...
        logger.debug(
            f"[gradio_gulliver.py] _kokoro_tts_subtitle - subtitle_file = {subtitle_file}, \
