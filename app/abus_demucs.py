...
def demucs_split_file(
    ...
    demucs_inst_file = os.path.join(
        temp_directory, demucs_model, file_name, "no_vocals.wav"
    )
    demucs_vocal_file = os.path.join(
        temp_directory, demucs_model, file_name, "vocals.wav"
    )
    ...
    inst_audio_file = os.path.join(
        output_dir, file_name + f"_{demucs_model}_inst." + audio_format
    )
    vocal_audio_file = os.path.join(
        output_dir, file_name + f"_{demucs_model}_vocal." + audio_format
    )
