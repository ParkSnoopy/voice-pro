...
class GradioAICover:
    ...
    def upload_source(
        ...
        try:
            ...
            logger.error(
                f"[gradio_rvc.py] upload_source - Error transcribing file: {e}"
            )
            gr.Warning(f"{e}")
            return None, None, None
    ...
    def make_cover(
        ...
        try:
            cover_audio_file = self._pipeline(
                rvc_voice,
                pitch_change,
                pitch_change_all,
                index_rate,
            ...
            cover_video_file = path_add_postfix(
                self.source_file, f"({rvc_voice} Ver)-{int(time.time())}"
            )
            if self.has_video:
                ffmpeg_replace_audio(
                    self.source_file, cover_audio_file, cover_video_file
                )
    ...
    def _pipeline(
        ...
        ai_vocals_path = os.path.join(
            output_dir,
            f"{os.path.splitext(os.path.basename(source_audio_file))[0]}_{rvc_voice}_p{pitch_change}_i{index_rate}_fr{filter_radius}_rms{rms_mix_rate}_pro{protect}_rmvpe.wav",
        )
        rvc_change_voice(
        ...
        if pitch_change_all != 0:
            instrumental_pitch_audio = path_add_postfix(
                self.fm.get_split("Instrumental.audio"), f"_p{pitch_change_all}", ".wav"
            )
            rvc_shift_pitch(
                self.fm.get_split("Instrumental.audio"),
                instrumental_pitch_audio,
                pitch_change_all,
            )
            instrumentals_path = instrumental_pitch_audio

            backup_pitch_audio = path_add_postfix(
                self.fm.get_split("BackupVocals.audio"), f"_p{pitch_change_all}", ".wav"
            )
            rvc_shift_pitch(
                self.fm.get_split("BackupVocals.audio"),
                backup_pitch_audio,
                pitch_change_all,
            )
        ...
        cover_audio_file = path_add_postfix(
            source_audio_file, f"({rvc_voice} Ver)-{timestamp}", f".{output_format}"
        )
        rvc_combine_audio(
            [ai_vocals_effect_path, backup_vocals_path, instrumentals_path],
            cover_audio_file,
            main_vocal_gain,
            backup_vocal_gain,
            inst_gain,
    ...
    def gradio_default_rvc(self):
        return [0, 0, 0.5, 3, 0.25, 0.33, 0, 0, 0, 0.15, 0.2, 0.8, 0.7]
