...
class GradioTranslate:
    ...
    def gradio_save(self, file_obj, source_lang, target_lang, src_text, target_text):
        ...
        if file_obj is not None:
            source_file = cmd_copy_file_to(
                file_obj.name, path_workspace_subfolder(file_obj.name)
            )
            source_text_file = path_add_postfix(
                source_file, f"translate-{time.time()}-{source_lang}.txt"
            )
            target_text_file = path_add_postfix(
                source_file, f"translate-{time.time()}-{target_lang}.txt"
            )

            self._write_file(source_text_file, src_text)
            self._write_file(target_text_file, target_text)
        else:
            source_text_file = os.path.join(
                path_translate_folder(), f"translate-{time.time()}-{source_lang}.txt"
            )
            target_text_file = os.path.join(
                path_translate_folder(), f"translate-{time.time()}-{target_lang}.txt"
            )
    ...
    def gradio_translate(self, text: str, source_lang, target_lang):
        ...
        input_text = text
        logger.debug(
            f"[gradio_translate.py] gradio_translate - input_text: {input_text}"
        )
        ...
        if subtitle_file:
            translated_file = self._translate_subtitle(
                subtitle_file, source_lang, target_lang
            )
    ...
    def _translate_subtitle(self, subtitle_file, source_lang, target_lang):
        try:
            ...
            self.translator.translate_file(
                source_lang, target_lang, subtitle_file, translated_file
            )
