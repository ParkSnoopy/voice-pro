...
def ffmpeg_replace_audio(
    input_path: str, audio_path: str, output_path: str, progress=gr.Progress()
):
...
def ffmpeg_extract_audio(input_path: str, output_path: str, audio_format: str = "wav"):
    ...
    command = (
        f'ffmpeg -y -i "{input_path}" -vn {encoding_options} "{output_path}" -nostdin'
    )
...
def ffmpeg_browser_compatible(input_path):
    ...
    if not os.path.exists(input_path):
        logger.error(
            f"[abus:ffmpeg_browser_compatible] Input file not found: {input_path}"
        )
        return False
    ...
    if not ffprobe_path:
        logger.error(
            "[abus:ffmpeg_browser_compatible] ffprobe not found in system PATH"
        )
        return False
    ...
    command = [
        "ffprobe",
        "-v",
        "quiet",  # 필요에 따라 -v error 등으로 변경 가능
        "-print_format",
        "json",
        "-show_streams",
        "-show_format",
    ...
    try:
        ...
        logger.debug(
            f"[abus:ffmpeg_browser_compatible] result.stdout = {result.stdout}"
        )
        ...
        if not video_info or not audio_info:  # video 또는 audio stream이 없는 경우
            logger.warning(
                f"[abus:ffmpeg_browser_compatible] No video or audio stream found in {input_path}"
            )
            return False
        ...
        # 브라우저 호환성 확인
        if (vcodec == "h264" and acodec == "aac") or (
            (vcodec == "vp8" or vcodec == "vp9")
            and (acodec == "opus" or acodec == "vorbis")
        ):
            logger.debug(
                "[abus:ffmpeg_browser_compatible] ffmpeg_browser_compatible: YES"
            )
            return True
        else:
            logger.debug(
                "[abus:ffmpeg_browser_compatible] ffmpeg_browser_compatible: NO"
            )
            return False
        ...
        logger.error(
            f"[abus:ffmpeg_browser_compatible] - Error occurred: Invalid JSON output from ffmpeg: {e}"
        )
        logger.error(
            f"[abus:ffmpeg_browser_compatible] - ffmpeg output: {result.stdout if 'result' in locals() else 'N/A'}"
        )  # ffmpeg 출력을 로깅
        return False
    except Exception as e:
        logger.error(f"[abus:ffmpeg_browser_compatible] - Error occurred: {e}")
...
def ffmpeg_mix_audio(
    ...
    if audio_format == "flac":
        ...
        encoding_options = (
            "-c:a libmp3lame -qscale:a 0 -ar 48000 -ac 2"  # -ar 48000 -ab 320k
        )
    elif audio_format == "ogg":
...
def ffmpeg_to_mono(
    input_path: str, left_path: str, right_path: str, audio_format: str = "wav"
):
...
def ffmpeg_trim_seconds(input_path: str, output_path: str, seconds: float):
    command = (
        f'ffmpeg -y -ss 00:00:00 -i "{input_path}" -t {seconds} -c copy "{output_path}"'
    )
...
def ffmpeg_video_resolution(input_path):
    ...
    if not os.path.exists(input_path):
        logger.error(
            f"[abus:ffmpeg_video_resolution] Input file not found: {input_path}"
        )
        return None
...
def ffmpeg_get_fps(input_path):
    ...
    input_path_encoded = input_path.encode("utf-8")  # 파일명 UTF-8로 인코딩
    command = ["ffmpeg", "-i", input_path_encoded]
    ...
    try:
        ...
        if target_size_mb:
            ...
            if original_size > target_size_mb:
                ...
                logger.info(
                    f"Adjusted CRF to {new_crf} for target size {target_size_mb}MB"
                )
