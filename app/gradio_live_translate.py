...
class ParallelTextGenerator:
        ...
        if self.source_language == self.target_language:
            ...
            for segment in segments or []:
                segment["text"] = self.translator.translate_text(
                    self.source_language, self.target_language, segment["text"]
                )
    ...
    def save(self):
        ...
        vtt_source_path = os.path.join(
            path_live_folder(), f"live-{time.time()}-{self.source_language}.vtt"
        )
        write_file(self.vtt_source, vtt_source_path)

        self.vtt_target = "WebVTT\n\n" + self.vtt_target
        vtt_target_path = os.path.join(
            path_live_folder(), f"live-{time.time()}-{self.target_language}.vtt"
        )
