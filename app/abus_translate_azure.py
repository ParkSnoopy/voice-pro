...
class AzureTranslator:
    def __init__(self) -> None:
        ...
        try:
            ...
            if exception.error is not None:
                logger.error(
                    f"Translation : {exception.error.message} ({exception.error.code})"
                )
    ...
    def get_language_code(self, language_name) -> str:
        ...
        for key, value in self.languages_dict.items():
            if (
                value.name.lower() == search_name
                or value.native_name.lower() == search_name
            ):
                return key
        return "en"
        ...
        try:
            ...
            if translation:
                for translated_text in translation.translations:
                    logger.debug(
                        f"[abus_translate_azure.py] request_translate - Text was translated to: '{translated_text.to}' and the result is: '{translated_text.text}'."
                    )
            ...
            if exception.error is not None:
                logger.error(
                    f"request_translate : {exception.error.message} ({exception.error.code})"
                )
            return "Error"

    def translate_text(
        self, source_lang: str, target_lang: str, text: str, progress=gr.Progress()
    ) -> str:
        ...
        for sentence in progress.tqdm(sentences, desc="Translating sentences..."):
            try:
                ...
                logger.debug(
                    f"[abus_translate_azure.py] translate_text - {source_lang}: {sentence} -> {target_lang}: {translated}"
                )
    ...
    def translate_file(
        ...
        tts_source_file = path_add_postfix(
            subtitle_file_path, f"-{source_lang}", ".srt"
        )
        ...
        for event in progress.tqdm(subs, desc="Translate..."):
            ...
            try:
                ...
                if translated_text:
                    ...
                    logger.debug(
                        f"[abus_translate_azure.py] translate_file : text       - {text}"
                    )
                    logger.debug(
                        f"[abus_translate_azure.py] translate_file : translated - {translated_text}"
                    )
                else:
                    logger.warning(
                        f"[abus_translate_azure.py] translate_file - Empty translation for: {text}"
                    )
            except Exception as e:
    ...
    def translate_text_webapi(
        ...
        params = {"api-version": "3.0", "from": azure_source, "to": azure_target}
