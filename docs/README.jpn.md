...
- **トラブルシューティング**: ほとんどの場合、`installer_files`フォルダを削除し、`configure.sh`を実行した後に`start.sh`を実行することで問題が解決します。
...
- Voice-Proは完全無料で、Linuxをサポートしています。
...
**configure.sh**と**start.sh**でVoice-Proを簡単にインストールできます。
...
### 3. アップデート
- 🚀 **update.sh**: Python環境を更新（再インストールより高速）

### 4. アンインストール
- **uninstall.sh**実行、またはフォルダ削除（ポータブルインストール）

## ❓ 使用のヒント

#### ブラウザが自動起動しない場合
- ターミナルを閉じ、**start.sh**を再実行するか
- ブラウザを直接起動し、ターミナルに表示されるアドレス（例: **http://127.0.0.1:7870**）を入力

#### CUDAメモリ不足エラーが出る場合
- システムモニターの「パフォーマンス」タブでGPUメモリを確認
