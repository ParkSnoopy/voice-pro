...
- **故障排除**: 在大多數情況下，刪除`installer_files`文件夾，然後依次運行`configure.sh`和`start.sh`即可解決問題。
...
使用**configure.sh**和**start.sh**輕鬆安裝Voice-Pro。
...
### 3. 更新
- 🚀 **update.sh**：更新Python環境（比重新安裝更快）

### 4. 解除安裝
- 執行**uninstall.sh**或刪除資料夾（可攜式安裝）

## ❓使用技巧

#### 瀏覽器沒有自動啟動時
- 關閉終端機，重新執行start.sh，或
- 直接啟動瀏覽器，在網址列輸入終端機顯示的網址（例如**http://127.0.0.1:7870**）

#### 出現CUDA記憶體不足錯誤時
- 在系統監視器-效能標籤中檢查GPU記憶體狀態
