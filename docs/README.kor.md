...
- NVIDIA GPU를 갖춘 Linux 환경을 대상으로 합니다.
...
- **문제 해결**: 대부분의 경우, `installer_files` 폴더를 삭제한 후 `configure.sh`를 실행하고 이어서 `start.sh`를 실행하면 문제가 해결됩니다.
...
**configure.sh** 및 **start.sh**으로 Voice-Pro를 쉽게 설치하세요.
...
### 3. 업데이트
- 🚀 **update.sh**: Python 환경 갱신 (재설치보다 빠름)

### 4. 제거
- **uninstall.sh** 실행 또는 폴더 삭제 (휴대용 설치)
...
- 터미널을 종료하고, start.sh 을 다시 실행하거나
- Browser를 직접 실행하고, 터미널에 표시된 주소(예, **http://127.0.0.1:7870** )를 주소창에 입력합니다.

#### CUDA Out-Of-Memory 오류가 발생하는 경우
- 시스템 모니터 - 성능 탭에서 GPU 메모리 상태를 확인하세요. 
